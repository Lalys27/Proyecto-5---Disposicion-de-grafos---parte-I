...
class GrafoGUI():
    ...
    def calcularDistancia(self):
        ...
        for oArista in self.set_arista:
            ...
            if distancia == 0:
                continue
            #print("Distancia:{p1}".format(p1=distancia), end = '')#Debug
            #print("Log:{p1}".format(p1=log(distancia/self.CONSTANT_C2)))#Debug
            ...
            #Calculo para visualizar los nuevos nodos
...
def pygame_start(grafo, ltAristas, title):
    """ 
    #Prueba
    ltAristas = [Arista(nodo_inicial=Nodo(num=1), nodo_final=Nodo(num=2)),
        Arista(nodo_inicial=Nodo(num=2), nodo_final=Nodo(num=4)),
        Arista(nodo_inicial=Nodo(num=3), nodo_final=Nodo(num=1)),
        Arista(nodo_inicial=Nodo(num=4), nodo_final=Nodo(num=1))]
    """ 
    ...
    while True:
        ...
        if grafoGUI.print_nodos:
            for node in grafoGUI.set_nodos:
                ...
                #pygame.display.flip()
                #pygame.time.delay(GRAFO_GUI.REFRESH)
                i_refresh = pygame_refresh(i_refresh, 5)
...
"""
oGrafo.modeloGrafoPygame(grafo = oGrafo, is_BASE = True)
